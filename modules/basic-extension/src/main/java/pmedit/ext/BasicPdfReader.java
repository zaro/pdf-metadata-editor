    ...
    public void loadFromPDF(PDDocument document, MetadataCollection mc) throws IOException, XmpParsingException, IllegalArgumentException {
        ...
        if (meta != null) {
            try {
                ...
                md.warnings.add(new MetadataInfo.Warning(MetadataInfo.Warning.Type.NORMAL, "All XMP metadata will be lost on save!", null));
            }
        }
        ...
        if (hasEncryption) {
            ...
            if(enc.getCOSObject().containsKey(COSName.LENGTH)){
                md.prop.keyLength = enc.getCOSObject().getInt(COSName.LENGTH);
            } else {
